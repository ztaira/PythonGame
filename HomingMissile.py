# This class is meant to be a homing missile

# =======================================================================
import Object


class HomingMissile(Object.Object):
    ...
    def update_position(self, up_press, down_press, left_press, right_press,
        ...
        if ((x_target-self.position[0])**2 + (y_target-self.position[1])**2)**0.5 < 20:
            self.to_delete = 1
            return self.position
        self.velocity_magnitude = (self.velocity[0]**2 +
                                   self.velocity[1]**2)**0.5
        # If velocity_magnitude < 10, missile accelerates
        if self.velocity_magnitude < 8:
            self.velocity[0] += self.ratio[0]*self.acceleration
            self.velocity[1] += self.ratio[1]*self.acceleration
        # Points velocity in direction of target
        self.velocity_magnitude = (self.velocity[0]**2+self.velocity[1]**2)**0.5
        self.ratio = [x_target-self.position[0], y_target-self.position[1]]
        self.ratio[0] /= ((x_target-self.position[0])**2 +
                          (y_target-self.position[1])**2)**0.5
        self.ratio[1] /= ((x_target-self.position[0])**2 +
                          (y_target-self.position[1])**2)**0.5
        self.velocity[0] = self.velocity_magnitude*self.ratio[0]
        self.velocity[1] = self.velocity_magnitude*self.ratio[1]
